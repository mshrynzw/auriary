...
- 0〜10の11段階評価（0=全くしない、10=たくさんする）
- 設定画面でデフォルト値を設定可能（`m_user_daily_defaults.exercise_level_default`）

**データ構造:**
- `t_diaries.exercise_level`（SMALLINT、0-10）に保存
- デフォルト値は `m_user_daily_defaults.exercise_level_default` から取得（デフォルト値は5）
...
**睡眠時間の計算:**
- 当日の就寝時間（`sleep_start_at`）と起床時間（`sleep_end_at`）を使用
- 日付をまたぐ場合（起床時刻が就寝時刻より前の場合）は、起床時刻を翌日に調整して計算
- 計算式: `睡眠時間 = 起床時刻 - 就寝時刻`（時間単位、小数点第1位まで）
- 負の値や24時間を超える値は除外
