...
- [FR-70] アプリをホーム画面にインストールできる（スマートフォンでネイティブアプリのように使用可能）
- [FR-71] オフライン時でも既存の日記を閲覧できる
- [FR-72] オフライン時に日記を下書き保存できる（オンライン復帰時に自動同期）
- [FR-73] オフライン時の動作をユーザーに通知できる
- [FR-74] アプリをスタンドアロンモードで起動できる（ブラウザUI非表示、ネイティブアプリと同様の起動体験）
