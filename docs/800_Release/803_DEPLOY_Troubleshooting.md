...
12. **Wrangler CLIログの確認**
    - Wrangler CLIでログを確認すると、静的ファイルが正常に読み込まれている場合があります
    - 例：`GET https://xxx.auriary.pages.dev/_next/static/chunks/xxx.js - Ok`
    - しかし、ブラウザのコンソールでは404エラーが表示される場合があります
    - この場合、以下の可能性があります：
      - **ブラウザのキャッシュ**: 古いデプロイメントのキャッシュが残っている
      - **デプロイメントURLの不一致**: 異なるデプロイメントURLにアクセスしている
      - **解決方法**:
        1. 最新のデプロイメントURLを確認（Cloudflare Dashboard → Deployments）
        2. ブラウザのキャッシュをクリア（Ctrl+Shift+R または Cmd+Shift+R）
        3. シークレットモードでアクセスして確認
        4. デベロッパーツール → Networkタブで「Disable cache」を有効にして再読み込み

13. **静的ファイルが部分的に読み込まれている場合**
    - Wrangler CLIログで一部の静的ファイルが「Ok」と表示されている場合、`_worker.js`は正常に動作しています
    - 404エラーが発生しているファイルは、以下の可能性があります：
      - ビルド時に生成されていない
      - パスが正しく設定されていない
      - ブラウザが古いキャッシュを使用している
    - **解決方法**:
      1. ブラウザのキャッシュをクリア
      2. 最新のデプロイメントURLにアクセス
      3. デベロッパーツール → Networkタブで、404エラーが発生しているファイルのリクエストURLを確認
      4. そのURLが正しいか確認（`/_next/static/...`の形式になっているか）
