...
├── layout.tsx          # ルートレイアウト
├── page.tsx            # トップページ
├── dashboard/          # ダッシュボードページ（実装済み）
│   ├── page.tsx        # ダッシュボードメイン
│   └── charts.tsx      # 統合グラフコンポーネント
├── diary/              # 日記関連ページ
│   ├── page.tsx        # 日記一覧
│   ├── [id]/           # 日記詳細・編集
│   └── new/            # 新規作成
...
graph LR
    ...
    Edge -->|CDN| Users[ユーザー]
    Vercel -->|API| Supabase[Supabase]
    Vercel -->|AI| OpenAI[OpenAI API]
```

**特徴：**

- **Next.js 最適化**：Vercel が Next.js を最適化してデプロイ
- **Edge Network**：グローバル CDN による高速配信
- **自動デプロイ**：GitHub へのプッシュで自動デプロイ
- **PWA対応**：Service Worker と manifest.json を配信（実装済み）

**実装詳細:**

- `package.json`: `build` スクリプトでビルド
